...
as $$
  ...
  left join public.vendedor v on v.id = h.vendedor_id and v.kiosco_id = h.kiosco_id
  where h.id = p_id
    and h.kiosco_id = public.current_kiosco_id();
$$;
...
as $$
  select jsonb_build_object(
    ...
    'detalles', coalesce((
      ...
      join public.item_venta iv on iv.id = vd.item_id and iv.kiosco_id = v.kiosco_id
      where vd.venta_id = v.id
        and vd.kiosco_id = v.kiosco_id
    ), '[]'::jsonb),
    'pagos', coalesce((
      select jsonb_agg(
        jsonb_build_object(
  ...
  left join public.vendedor ven on ven.id = v.vendedor_id and ven.kiosco_id = v.kiosco_id
  where v.id = p_id
...
as $$
  ...
  from public.horario h
  where h.kiosco_id = public.current_kiosco_id();
$$;
...
begin
  ...
  insert into public.horario (vendedor_id, hora_ingreso, hora_egreso, kiosco_id)
  values (p_vendedor_id, coalesce(p_hora_ingreso, now()), null, v_kiosco_id)
...
begin
  ...
  insert into public.movimiento_stock (products_move_stock, move_type, deleted, kiosco_id)
  values (v_products, v_move_type, false, v_kiosco_id)
...
begin
  ...
  insert into public.venta (cliente_id, vendedor_id, total, deleted, kiosco_id)
  values (v_cliente_id, v_vendedor_id, round(v_total_detalles::numeric, 2), false, v_kiosco_id)
  ...
  loop
    ...
    insert into public.item_venta (codigo, nombre, descripcion, precio, cantidad, kiosco_id)
    values (
      v_producto_codigo,
    ...
    values (v_venta_id, v_item_id, v_kiosco_id);
  end loop;
  ...
  loop
    ...
    where id = v_metodo_id
      and deleted = false
      and (kiosco_id = v_kiosco_id or kiosco_id is null);
    ...
    values (v_venta_id, v_metodo_id, v_monto, v_cuotas, v_kiosco_id);
